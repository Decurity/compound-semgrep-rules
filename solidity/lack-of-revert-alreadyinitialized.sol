...
contract Configurator is ConfiguratorStorage {
     ...
     **/
    // ok: lack-of-revert-alreadyinitialized
    ...
    // ok: lack-of-revert-alreadyinitialized
    function initialize(address governor_) public {
        if (version != 0) revert AlreadyInitialized(governor_);
        if (governor_ == address(0)) revert InvalidAddress();
    ...
    // ok: lack-of-revert-alreadyinitialized
    function initialize(address governor_) internal {
        if (governor_ == address(0)) revert InvalidAddress();
    ...
    // ruleid: lack-of-revert-alreadyinitialized
    function initialize(address governor_) public {
        if (governor_ == address(0)) revert InvalidAddress();
